...
<div class="wrap">
	...
	<?php if (isset($this->photo)) { ?>
		...
		<form name="configuration" method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>" enctype="multipart/form-data">
			<?php wp_nonce_field(AefPhotosContestAdmin::PAGE_PHOTO_EDIT . $this->photo['id'],
				AefPhotosContestAdmin::PAGE_PHOTO_EDIT . '_nonce') ?>
	...
	<?php submit_button(); ?>
		...
		<?php if (!empty($this->photo['photo_mime_type'])) { ?>
						...
						<table>
							<tr>
								<td rowspan="4" >
									<a href="<?php echo $aefPC->getPhotoUrl($this->photo,
				'view') ?>?t=<?php echo time() ?>">
										<img style="float: left" src="<?php echo $aefPC->getPhotoUrl($this->photo,
				'thumb') ?>?t=<?php echo time() ?>" />
									</a>
								</td>
							</tr>
							<tr>
								<td style="padding-left: 12px"><a href="<?php echo $aefPC->getPhotoUrl($this->photo,
